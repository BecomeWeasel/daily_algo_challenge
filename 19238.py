    ...
    for k in range(4):
      ...
      if not check[npos_y][npos_x] and grid[npos_y][npos_x] != 1:
        ...
        if grid[npos_y][npos_x] < 0:
          heapq.heappush(pq,
                         (dist + 1, npos_y, npos_x, grid[npos_y][npos_x] * -1))
...
for i in range(1, M + 1):
  pass_ori_y, pass_ori_x, pass_dest_y, pass_dest_x = map(
      int,
...
while pass_count < M:
  ...
  # 탑승한 손님의 목적지 위치를 구해옴
  pass_dest_y, pass_dest_x = passengers[passenger_number][3], passengers[
      passenger_number][4]
