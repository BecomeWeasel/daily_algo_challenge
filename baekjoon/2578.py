      ...
      for i in range(5):
        for j in range(5):
          if bingo_map[i][j][0] == number:
            ...
            if sum(bingo_map[i][h][1] for h in range(5) if bingo_map[i][h][1]==True)==5:
              bingo_counter += 1

            # 세로로 확인

            if sum(bingo_map[h][j][1] for h in range(5) if bingo_map[h][j][1]==True)==5:
              bingo_counter += 1
...
def dia_bingo_check(bingo_map):
  ...
  return (1 if sum(bingo_map[h][h][1] for h in range(5) if bingo_map[h][h][1]==True)==5 else 0) + (1 if sum(bingo_map[0+h][4-h][1] for h in  range(5) if bingo_map[0+h][4-h][1]==True)==5 else 0)
