...
def sol():
        ...
        for i in range(N):
            for j in range(N):
                ...
                if board[i][j] == 'B' and i + 1 < N and board[i + 1][j] == 'B':
                    tree = log(True, (i + 1, j))
                    return tree
                elif board[i][j] == 'B' and j + 1 < N and board[i][j +
                                                                   1] == 'B':
    ...
    while q:
        ...
        if head.is_vertical:
            # 위로 갈때
            if (0<=c_y-1-1<N) and not visit[True][c_y-1][c_x] and board[c_y-1-1][c_x]!='1':
            ...
            if (0<=c_x+1<N) and not visit[True][c_y][c_x+1] and board[c_y-1][c_x+1]!='1' and board[c_y][c_x+1]!='1' and board[c_y+1][c_x+1]!='1':
            ...
            # 위로 갈때
            if (0<=c_y-1<N) and not visit[False][c_y-1][c_x] and board[c_y-1][c_x-1]!='1' and board[c_y-1][c_x]!='1' and board[c_y-1][c_x+1]!='1':
            ...
            if (0<=c_y+1<N) and not visit[False][c_y+1][c_x] and board[c_y+1][c_x-1]!='1' and board[c_y+1][c_x]!='1' and board[c_y+1][c_x+1]!='1':
                ...
                q.append((log(False,(c_y+1,c_x)),count+1))
            # 우로 갈때
            if (0<=c_x+1+1<N) and not visit[False][c_y][c_x+1] and board[c_y][c_x+1+1]!='1':
