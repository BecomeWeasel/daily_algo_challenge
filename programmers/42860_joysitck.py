        ...
        while len(incorrect_char_idx) != 0:
            ...
            for idx in incorrect_char_idx:
                ...
                # 두 방법 중 짧은것을 택해야함
                    ...
                    distance_list.append(min((current_pos - idx) % len(name), idx - current_pos))
...
def solution2(name):
    ...
    direct = (
        sum([j[0] - i[0] for i, j in zip(incorrect_char_and_pos[:-1], incorrect_char_and_pos[1:])])
    ...
    for i in range(len(incorrect_char_and_pos) - 1, 0, -1):
        if incorrect_char_and_pos[i - 1][0] != 0:
            reverse_direct += incorrect_char_and_pos[i][0] - incorrect_char_and_pos[i - 1][0]
        else:
            break

    return min(direct, reverse_direct) + sum(x[1] for x in incorrect_char_and_pos)
